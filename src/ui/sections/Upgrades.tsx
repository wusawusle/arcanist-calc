  ...
  return (
    <tr role="row" className={row.level >= max ? 'maxed' : undefined}>
      ...
      {row.priced ? (
        ...
        <td role="cell" className="cost"><span className="note">Unknown cost</span></td>
      )}
    </tr>
  );
}
  ...
  return (
    <Section title="Spells" icon={SECTION_ICONS.spells} eyebrow="runes" flush>
      ...
      <p className="note" style={{ padding: '10px 16px 14px' }}>
        ...
        10 Drift), but the supplied page has no potency-cost table for it. Diggy Diggy Hole, Blue Giant, Rainbow Road, Party Fever and Bomb's Blessing still have ??? effect values on the supplied page, so those effects are shown as pending rather than guessed.
      </p>
    </Section>
  );
}
...
export function Exchange({ result, update }: Props) {
  return (
    <Section
      ...
      eyebrow="13 released + WIP spell power"
      help="exchange"
      flush
    >
      <div className="scroll-x">
