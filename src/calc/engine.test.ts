...
describe('exchange upgrades use the current wiki costs', () => {
  ...
  it('models all thirteen released Exchange upgrades', () => {
    expect(EXCHANGE_UPGRADES.map((def) => def.id)).toEqual(ids);
    expect(fresh.rows.exchange).toHaveLength(13);
    expect(fresh.rows.exchange.every((row) => row.priced)).toBe(true);
  });
