...
function computeSpells(input: ArcanistInput, ext: ExternalBonuses, derived: DerivedBonuses) {
  ...
  for (const id of SPELL_IDS) {
    ...
    const arcanistSpellPower = input.exchange.arcanistSpellPower * 0.005;

    outcomes[id] = {
      id,
      unlocked,
      primary: spellEffect(def.primary.base, unlocked, level, rank, cardBonus, spellPower, arcanistSpellPower),
      secondary: spellEffect(def.secondary.base, unlocked, level, rank, cardBonus, spellPower, arcanistSpellPower),
...
function computeStats(
  ...
  return {
    ...
    attackInterval: BASE_STATS.attackInterval / (1 + effects.attackSpeed),
    ...
    stunNegate: Math.min(1, effects.stunNegate + effects.allDebuffNegate),
    shinyChance:
    ...
    superShinyBonus: BASE_STATS.superShinyBonus + effects.superShinyLoot,
    ultraShinyChance: effects.ultraShinyChance,
    ultraShinyBonus: BASE_STATS.ultraShinyBonus,
    weakenNegate: Math.min(1, effects.weakenNegate + effects.allDebuffNegate),
    dazeNegate: Math.min(1, effects.dazeNegate + effects.allDebuffNegate),
    brittleChance:
...
function computeAverages(stats: Stats): Averages {
  const { shinyChance, superShinyChance, ultraShinyChance, shinyBonus, superShinyBonus, ultraShinyBonus } = stats;
  const shinyTable: WeightedOutcome[] = [
    ...
    {
      label: 'super shiny',
      chance: shinyChance * superShinyChance * (1 - ultraShinyChance),
      value: shinyBonus + superShinyBonus,
    },
    {
      label: 'ultra shiny',
      chance: shinyChance * superShinyChance * ultraShinyChance,
...
function lootRange(
  ...
  switch (type) {
    case 'soft':
      return {
        min: block.baseMinLoot + effects.allMinLoot,
        max: block.baseMaxLoot + effects.softMaxLoot + effects.allMaxLoot + shared,
      };
    case 'dense':
      return {
        min: block.baseMinLoot + effects.allMinLoot,
        max: block.baseMaxLoot + effects.denseMaxLoot + effects.allMaxLoot + shared,
      };
    case 'jagged':
      return {
        min: block.baseMinLoot + effects.jaggedMinLoot + effects.allMinLoot,
        max: block.baseMaxLoot + effects.jaggedMaxLoot + effects.allMaxLoot + shared,
      };
  }
}
...
function computeEssence(
  ...
  const weakenChance = block.weakenChance * (1 - stats.weakenNegate);
  const avgWeaken =
    1 - weakenChance * block.weakenDuration + weakenChance * block.weakenDuration * block.weakenMulti;
  const avgRegen = Math.max(0, block.regen - effects.regenReduction) / block.regenInterval;
  ...
  const cycleTime = timeToMine + Math.max(0, block.respawn + effects.respawnReduction);
  ...
  const luckiestLoot =
    ...
    (stats.shinyChance > 0 && stats.superShinyChance > 0 && stats.ultraShinyChance > 0 ? stats.ultraShinyBonus : 0);
